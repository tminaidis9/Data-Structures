...
void list_remove(List list, ListNode node) {
	ListNode previous = node;
		...
		for (ListNode node2 = list->dummy; node2 != NULL; node2 = node2->next){
			if (node2->next == node)
			  previous = node2;
	...
	// Σύνδεση του previous κομβου με τον επομενο του node
...
void list_append(List list, List to_append) {
	...
	for(ListNode node = to_append->dummy->next ; node != LIST_EOF ; node = node->next){
	new = malloc(sizeof(*new));
	new->value = list_node_value(to_append,node);
	list->last->next = new;
	list->last = new;
	list->size++;
